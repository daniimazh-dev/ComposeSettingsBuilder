...
class RangeSlider internal constructor(
...
) : ComposeSetting<ClosedFloatingPointRange<Float>>(depends = depends, initialEnabled = enabled, initialVisible = visible) {
    ...
    var sliderState: MutableState<RangeSliderState> = mutableStateOf(
        RangeSliderState(
            ...
            onValueChangeFinished = {
                val newValue = sliderState.value.activeRangeStart..sliderState.value.activeRangeEnd
    ...
    override fun saveLogic(): com.daniil.csb.persistence.SaveSettingPackage {
        return com.daniil.csb.persistence.SaveSettingPackage.PairPackage(
            id,
            enabled.value,
            value.value.let { it.start.toString() to it.endInclusive.toString() })
    }

    override fun loadLogic(pack: com.daniil.csb.persistence.SaveSettingPackage) {
        enabled(pack.enable)
        if (isSaveSetting) {
    ...
    override fun changeValue(newValue: ClosedFloatingPointRange<Float>) {
        ...
        sliderState.value = RangeSliderState(
            ...
            valueRange = range,
            onValueChangeFinished = {
                val updatedValue = sliderState.value.activeRangeStart..sliderState.value.activeRangeEnd
    ...
    override fun UI(modifier: Modifier, position: GroupItemClip?) {
        ...
        DefaultSettingUI(
            ...
            icon = icon,
            badge = badge,
            title = { if (title.isNotBlank()) Text(text = translator.translate(title), style = style.titleStyle) },
            description = { description?.let { Text(text = translator.translate(it), style = style.descriptionStyle) } },
            action = {},
            display = {
                Column(
